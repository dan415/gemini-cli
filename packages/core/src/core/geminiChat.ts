...
export class GeminiChat {
   ...
   * mutator function. This may involve emitting trimming parts from a chunk
   * as well as omtting some chunks altogether.
   *
   * We do this because - especially in the context of parallel tool calls -
   * it improves tool call quality if the model gets feedback from one mutating
   * function call before it makes the next one.
   */
      ...
      for (const part of content.parts) {
        if (this.isMutatorFunctionCall(part)) {
          if (foundMutatorFunctionCall) {
            ...
            // Clip the parts and end streaming.
